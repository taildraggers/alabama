"""Aggregates Alabama-located listings from every taildraggers.com
...
ones whose location ends in ", AL".
...
PAGE_TITLE = "Taildragger Ads in Alabama"
STATE_ABBR = "AL"
...
_ALABAMA_LOCATION_RE = re.compile(r",\s*AL\s*$", re.IGNORECASE)


def _is_alabama(location: str) -> bool:
    return bool(_ALABAMA_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        alabama_rows = [row for row in rows if _is_alabama(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(alabama_rows)} in Alabama")
        all_listings.extend(alabama_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Alabama listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Alabama listings: {len(listings)}")
