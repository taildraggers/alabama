...
Daily aggregator of Alabama-located taildragger classified listings,
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, AL` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Alabama"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/alabama/`
...
```html
<iframe
  src="https://taildraggers.github.io/alabama/"
  title="Taildragger Ads in Alabama"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_ALABAMA_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
